...
\item B: No
\end{itemize}
\item Suppose two loaded train cars are moving toward one another, and they couple together.  The first has a mass of $1.50\times 10^5$ kg and a velocity of $0.30\hat{i}$ m s$^{-1}$, and the second has a mass of $1.10\times 10^5$ kg and a velocity of $-0.12\hat{i}$ m s$^{-1}$. What is their final velocity? \\ \vspace{5cm}
\item A proton collides with a neutron (with essentially the same mass as the proton) to form a particle called a deuteron (see Fig. \ref{fig:deu}). What is the velocity of the deuteron if it is formed from a proton moving with velocity $7.0 \times 10^{6}$ m/s to the right and a neutron moving with velocity $-4.0 \times 10^{6}$ m/s to the left? \\ \vspace{5cm}
