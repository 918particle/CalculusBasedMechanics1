...
\begin{equation}
\boxed{
\tau = \mu \times \vec{B} \label{eq:torque}
}
\end{equation}
In Eq. \ref{eq:torque}, the quantity $\vec{mu} = I\vec{A}$ is called the \textit{magnetic moment.} \\ \vspace{0.2cm}
\textbf{Now that we can change voltage to torque, all we need for a motor is a uniform magnetic field.  How do we make one of those...?}
\end{frame}
