...
\item In a fourth column, compute the base-10 logarithm of $r^2$ times the field strength.  In a fifth column, compute the base-10 logarithm of the distance.
\end{enumerate}
\end{frame}
...
\textbf{Click and drag a positive charge into the 2D coordinate system.  This is analagous to charging an insulator.}
\begin{enumerate}
\item Plot $\log_{10}(r^2 E)$ vs. $\log_{10}(r)$ and estimate the slope.  How close to zero do you think it is?  What are some sources of error that contribute to the uncertainty in the slope?
...
\begin{itemize}
\item +1
\item The positive charges remain in place but the negative charges move away from the rod.
...
\item Both A and B
\item 0
\item The electric field is zero inside the conductor.
\end{itemize}
\end{column}
\end{columns}
\end{frame}
